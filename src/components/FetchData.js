...
export default function getData(content, lastValue) {
  ...
  if (lastValue)
    return axios.get(
      `${url}?ts=${currentTime}&apikey=${publicKey}&hash=${hash}&offset=${lastValue}`
    );
  return axios.get(`${url}?ts=${currentTime}&apikey=${publicKey}&hash=${hash}`);
}
